...
ASSET_URL="https://api.github.com/repos/WebAssembly/binaryen/releases/assets/${ASSET_ID}"
FALLBACK_URL="https://github.com/WebAssembly/binaryen/releases/download/version_${BINARYEN_VERSION}/${ARCHIVE}"
TEMP_DIR=$(mktemp -d)
...
if ! curl -fL \
  ...
  "$ASSET_URL" \
  -o "$TEMP_DIR/$ARCHIVE"; then
  echo "GitHub release asset API failed; retrying the pinned browser URL" >&2
  curl -fL \
    --retry 4 \
    --retry-delay 2 \
    --retry-all-errors \
