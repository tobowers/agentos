 ...
 */
describe.skipIf(!existsSync(VIM_BINARY))(
	"vim full-screen rendering (strict)",
	() => {
		...
		it("lays out the screen with the status line on the bottom row", async () => {
			...
			if (process.env.AGENTOS_VIM_FIXTURE_BIN) {
				...
				copyFileSync(
					process.env.AGENTOS_VIM_FIXTURE_BIN,
					join(dir, "bin", "vim"),
				);
				writeFileSync(
			...
			expect(
				opened[0],
				"row 0 must not carry the file message + tilde",
			).not.toContain("render.txt");
			...
			expect(
				inserting[0],
				"typed text lands on the first content row",
			...
			expect(
				inserting[ROWS - 1],
				"text must not be echoed onto the status row",
			...
			const content = Buffer.from(
				await vm.readFile("/work/render.txt"),
			).toString("utf8");
